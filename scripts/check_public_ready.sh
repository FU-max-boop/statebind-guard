...
echo "[1/9] Checking for obvious local paths"
...
echo "[2/9] Checking for common secret strings"
...
echo "[6/9] Running schema compatibility check"
make schema-check >/dev/null

echo "[7/9] Running benchmark suite"
make benchmark >/dev/null

echo "[8/9] Running repository smoke test"
bash scripts/run_smoke_test.sh >/dev/null

echo "[9/9] Running package CLI smoke"
