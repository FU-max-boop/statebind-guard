...
jobs:
  statebind-guard:
    ...
    steps:
      ...
      - id: statebind
        uses: FU-max-boop/statebind-guard@v0.1.18
        with:
          handoff: HANDOFF.md
...
`FU-max-boop/statebind-guard@v0.1.18`.
...
The generated `statebind init` workflow and this repository's dogfood workflow
also upload the SARIF file through `github/codeql-action/upload-sarif@v3`, so
StateBind findings can appear in GitHub code scanning instead of only as
...
`passed=false`, `errors=1`, `warnings=1`, `exit_code=1`, JSON, SARIF,
Markdown, and HTML reports. This keeps failure routing testable for downstream
workflows.
...
clean wheel package smoke across Python 3.10 through 3.13 on Ubuntu and macOS,
the demo, the proof command, and privacy-oriented public-release checks.
